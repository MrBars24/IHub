...
/** 
 * This is a modified version of FacebookBridge from rss-bridge package
 * 
...
class FacebookBridge extends \BridgeAbstract {
 ...
 please insert the parameter as follow : myExamplePage/132621766841117';
	...
	protected function extractFromDelimiters($string, $start, $end){
	...
	public function collectData(){
		...
		$unescape_fb_link = function($matches){
			if(is_array($matches) && count($matches) > 1) {
				...
				if(strpos($link, 'facebook.com/l.php?u=') !== false)
					$link = urldecode($this->extractFromDelimiters($link, 'facebook.com/l.php?u=', '&'));
				return ' href="' . $link . '"';
			}
		};
		...
		if($this->getInput('u') !== null) {
			$page = 'https://web.facebook.com/' . $this->getInput('u');
...
EOD;
		...
		$requestString = 'https://web.facebook.com/pages_reaction_units/more/?page_id='
		. $pageID
		...
		if (!$html) {
			throw new \Exception('Cannot build the html content from the url provided');
		}
		...
		foreach($html->find("article") as $content) {
			...
			$item['uri'] = "http://web.facebook.com"
	...
	private function computeNextLink($string, $pageID){
		...
		return implode(
			'',
			array(
				"https://web.facebook.com/pages_reaction_units/more/?page_id=",
				$pageID,
