...
class GigAttachment extends Model
{
	// App
	use CommonTrait;
	 ...
	 * @var string
	 */
	protected $table = 'gig_attachment';
	 ...
	 * @var array
	 */
	protected $appends = ['media_path', 'media_path_large', 'media_path_thumb'];
	...
	public function getMediaPathThumbAttribute()
	{		
		if($this->type == 'image' && !is_null($this->file)) {
			return route('general::thumbnail', ['template' => 'medium_ratio', 'file_path' => $this->file->relative_web_path]);
		}
		return $this->media_path;
	}

	/// Section: AttachmentTrait

	protected $owner = Gig::class;
	...
	// @todo: move this to a trait along with the PostAttachment@storeFile
